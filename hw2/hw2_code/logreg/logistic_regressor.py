...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        for i in range(len(y)):
            J=J+1.0/len(y)*(-y[i]*math.log(utils.sigmoid(np.dot(X[i,:],theta)))-(1-y[i])*math.log(1-utils.sigmoid(np.dot(X[i,:],theta))))
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
    ...
    def grad_loss(self, *args):
        ...
        grad=np.dot((utils.sigmoid(np.dot(X,theta))-y), X)/len(y)
...
#        for i in range(len(y)):
#            J=J+1.0/len(y)*(-y[i]*math.log(utils.sigmoid(np.dot(X[i,:],theta)))-(1-y[i])*math.log(1-utils.sigmoid(np.dot(X[i,:],theta))))
#        J=J+reg/len(y)/2*np.sum(theta**2)
        J=-np.dot(np.log(utils.sigmoid(np.dot(X,theta))),y)/float(len(y))-np.dot((np.log(1-utils.sigmoid(np.dot(X,theta)))),(1-y))/float(len(y))+reg*np.sum(theta**2)/2/len(y)
...
#        grad[0]=(np.dot((utils.sigmoid(np.dot(X,theta))-y), X)/len(y))[0]
#        for i in range(1,len(theta)):
#            grad[i]=(np.dot((utils.sigmoid(np.dot(X,theta))-y), X)/len(y))[i]+reg/len(y)*theta[i]
        grad=np.dot((utils.sigmoid(np.dot(X,theta))-y), X)/len(y)+reg/len(y)*np.vstack([[0],theta[1:len(theta)].reshape(len(theta)-1,1)]).reshape(1,len(theta))[0]
