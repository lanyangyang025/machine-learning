...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
  ...
  #      grad = np.zeros((dim,))
        ...
        J=np.sum((hypothesis-y)**2)/(2*num_examples)+np.sum(theta**2)*reg/(2*num_examples)
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        grad[0]=np.dot((np.dot(X,theta)-y),X[:,0])/(len(y))
        for num in range(1,dim):
             grad[num]=np.dot((np.dot(X,theta)-y),X[:,num])/(len(y))+reg*theta[num]/len(y)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return grad
